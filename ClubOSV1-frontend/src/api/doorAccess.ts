...
export const doorAccessAPI = {
  ...
  unlock: async (params: UnlockParams) => {
    const response = await http.post('/door-access/unlock', params);
    return response.data;
  },

  // Lock a door
  lock: async (params: LockParams) => {
    const response = await http.post('/door-access/lock', params);
    return response.data;
  },

  // Get door status for a location
  getStatus: async (location: string) => {
    const response = await http.get(`/door-access/status/${location}`);
    return response.data;
  },

  // Emergency actions
  emergency: async (params: EmergencyParams) => {
    const response = await http.post('/door-access/emergency', params);
    return response.data;
  },
  ...
  getLogs: async (location: string, doorKey?: string, limit: number = 20) => {
    ...
    const response = await http.get(`/door-access/logs/${location}?${params}`);
    return response.data;
  },

  // Get available doors for a location
  getDoors: async (location: string) => {
    const response = await http.get(`/door-access/doors/${location}`);
    return response.data;
  }
};