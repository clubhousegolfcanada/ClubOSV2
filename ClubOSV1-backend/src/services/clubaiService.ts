  ...
  try {
    ...
    const tools = restartEnabled ? [{
      ...
      function: {
        ...
        description: 'Trigger a remote restart of a TrackMan simulator. Only call this AFTER the customer has confirmed they want a restart AND provided their location and box number.',
        parameters: {
    ...
    if (choice?.tool_calls && choice.tool_calls.length > 0) {
      ...
      if (toolCall.function?.name === 'restart_trackman') {
        ...
        // Return the function call alongside any text response
