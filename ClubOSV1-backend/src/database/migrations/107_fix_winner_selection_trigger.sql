-- UP
-- Fix the check_challenge_winner_agreement trigger to use correct column name
...
-- DOWN
-- Revert to the previous (broken) version
CREATE OR REPLACE FUNCTION check_challenge_winner_agreement() 
RETURNS TRIGGER AS $$
DECLARE
  v_creator_id UUID;
...
BEGIN
  ...
  -- Get both players' selections (using wrong column name: winner_id)
  SELECT 
