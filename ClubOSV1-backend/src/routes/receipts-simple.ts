  ...
  try {
    ...
    const summaryQuery = `
      SELECT
        ...
        COALESCE(SUM(COALESCE(hst_cents, tax_cents, 0)), 0) as total_hst_cents,
  ...
  try {
    ...
    if (exportFormat === 'csv') {
      ...
      const csvData = receipts.map((receipt: any) => ({
        ...
        'HST': (receipt.hst_cents || receipt.tax_cents) ? ((receipt.hst_cents || receipt.tax_cents) / 100).toFixed(2) : '',
      ...
      const csvData = receipts.map((receipt: any) => ({
        ...
        'Vendor': receipt.vendor || '',
        'Amount': receipt.amount_cents ? (receipt.amount_cents / 100).toFixed(2) : '0.00',
        'Tax': (receipt.tax_cents || receipt.hst_cents) ? ((receipt.tax_cents || receipt.hst_cents) / 100).toFixed(2) : '0.00',
        'Category': receipt.category || '',
        'Payment Method': receipt.payment_method || '',
        'Location': receipt.club_location || '',
        'Personal Card': receipt.is_personal_card ? 'Yes' : 'No',
    ...
    try {
      ...
      const insertOneReceipt = async (
        ...
        `, [
          ...
          ocrResult?.hstAmount ? Math.round(ocrResult.hstAmount * 100) : (ocrResult?.taxAmount ? Math.round(ocrResult.taxAmount * 100) : null),
